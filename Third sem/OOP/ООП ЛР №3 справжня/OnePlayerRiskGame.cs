    ...
    public class OnePLayerRiskGame : BaseGame
    {
        ...
        protected override void UpdateInfo(UsersEntity WinnerPlayer, UsersEntity LoserPlayer, UserService userService)
        {
            if(RiskPlayer == WinnerPlayer.UserName)
            {
                ...
                LoserPlayer = LoserPlayer.SetUsersEntity(LoserPlayer.UserName, LoserPlayer.typeOfUsers, LoserPlayer.CurrentRaiting, LoserPlayer.GamesCount + 1, LoserPlayer.countOfWinningGames, LoserPlayer.countOfLostGames + 1);
            ...
            else if(RiskPlayer == LoserPlayer.UserName)
            {
                ...
                userService.UpdateAccountInfo(LoserPlayer, LoserPlayer.UserName);
                WinnerPlayer = WinnerPlayer.SetUsersEntity(WinnerPlayer.UserName, WinnerPlayer.typeOfUsers, WinnerPlayer.CurrentRaiting, WinnerPlayer.GamesCount + 1, WinnerPlayer.countOfWinningGames + 1, WinnerPlayer.countOfLostGames);
                userService.UpdateAccountInfo(WinnerPlayer, WinnerPlayer.UserName);
            }
        }
    }
